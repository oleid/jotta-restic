        ...
        loop {
            match reader.read_event(&mut buf)? {
                Event::Start(element) => {
                    ...
                    match element.name() {
                        b"path" => {
                            file.path = element_text(reader)?.expect("Couldn't parse path of file")
                        }
                        b"abspath" => (),
                        b"currentRevision" => (), // ignore begin of revision tag
                        b"number" => {
                            file.revision =
                                element_usize(reader)?.expect("Couldn't parse revision of file")
                        }
                        ...
                        b"size" => {
                            file.size = element_usize(reader)?.expect("Couldn't parse size of file")
                        }
                        b"mime" => file.mime = element_mime(reader)?,
                        b"md5" => {
                            file.md5 =
                                element_text(reader)?.expect("Couldn't parse md5 sum of file")
                        }
